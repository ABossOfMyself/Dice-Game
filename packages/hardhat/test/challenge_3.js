...
describe("🚩 Challenge 3: 🎲 Dice Game", function () {
  ...
  async function getRoll(getRollLessThanThree) {
    let expectedRoll;
    while (true) {
      ...
      if (expectedRoll.lte(2) == getRollLessThanThree) {
        break;
      }
  ...
  describe("🔑 Rigged Rolls", function () {
    it("Should call diceGame.rollTheDice for a roll <= 2", async () => {
      const getRollLessThanThree = true;
      const expectedRoll = await getRoll(getRollLessThanThree);
    ...
    it("Should not call diceGame.rollTheDice for a roll > 2", async () => {
      const getRollLessThanThree = false;
      const expectedRoll = await getRoll(getRollLessThanThree);
